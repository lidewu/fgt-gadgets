...
class FDSReqObj:
    ...
    def parse_raw_data(self):
        try:
            ...
            self.payload_len, _, self.sys_ver, _, self.payload_crc32, self.header_crc32 = struct.unpack("<II8s56sII", header2)
            ...
            print(f"[-] Error parsing FDSReqObj: {e}")
            raise
    ...
    def pack_obj(self):
        try:
            ...
            self.raw_data += struct.pack("4s20s1s23s", self.magic, self.obj_name.ljust(20, b"\x00"), self.fixed_str, b"\x00" * 23)
            self.raw_data += struct.pack("<II8s56sI", self.payload_len, self.header_len, self.sys_ver, b"\x00" * 56, self.payload_crc32)
            ...
            print(f"[-] Error packing FDSReqObj: {e}")
            raise
...
class FDSReq:
    ...
    def parse_raw_data(self):
        try:
            ...
            print(f"[-] Error parsing FDSReq: {e}")
            raise
    ...
    def pack_req(self):
        try:
            ...
            self.raw_data += struct.pack("<4s8sIII12s24s", self.magic, self.sys_ver, self.fixed_int, self.payload_len,
                                         self.header_len, self.time_str, b"\x00" * 24)
            self.header_crc32 = zlib.crc32(self.raw_data + b"B1gS")
            self.raw_data += struct.pack("<I", self.header_crc32)
            self.raw_data += self.payload
            return self.raw_data
        except Exception as e:
            print(f"[-] Error packing FDSReq: {e}")
            raise
...
def start_server():
    try:
        # Create SSL context with broader protocol range for debugging
        context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
        context.minimum_version = ssl.TLSVersion.TLSv1  # Allow TLS 1.0 for debugging
        context.maximum_version = ssl.TLSVersion.TLSv1_3
        # Lower security level to allow legacy signature algorithms
        context.set_ciphers('DEFAULT@SECLEVEL=0')
        context.options |= ssl.OP_NO_SSLv2 | ssl.OP_NO_SSLv3  # Explicitly disable SSLv2/v3
        print(f"[*] SSL Context: min_version={context.minimum_version}, max_version={context.maximum_version}")
        print(f"[*] Configured ciphers: {context.get_ciphers()}")
        ...
        while True:
            try:
                ...
                print(f"[-] SSL error during client connection: {ssl_err}")
            except Exception as e:
